...
### 1. [VERIFY-NEEDED] Firmware version baseline says `v0.3.1-stream`, but a whole new firmware capability ("Tier-1 firmware: leaves advertise BLE → ground-truth inter-leaf calibration matrix") has landed since
**Status note:** Added VERIFY comment next to leaves heading; needs Vlad/Ethan to confirm flashed version.
...
### 2. [FIXED] "First planned capability: distributed BLE sensing" is way past tense
**Status note:** Rewrote intro to "Shipped capability" with the actual shipped list.
...
### 3. [FIXED] "Next moves on deck" list is almost entirely done
**Status note:** Rewrote the whole section against the Q3 PRDs + outstanding cleanup items.
...
### 4. [FIXED] "Future capabilities" list is partially shipped
**Status note:** Moved trilateration + dashboard into a new "Shipped capabilities" section; future list now points at the 3 Q3 PRDs.
...
### 5. [FIXED] Services directory inventory in CLAUDE.md is silent on calibration + tracks modules
**Status note:** Expanded Pi-services bullet into a full inventory including dashboard, calibration, tracks.
...
### 6. [FIXED] The repo has TWO firmware trees and CLAUDE.md is muddy about which is live
**Status note:** Intro now names `firmware/esp32-wroom-mockingbird/` as the live tree.
...
### 7. [VERIFY-NEEDED] Pi IP `192.168.8.202` is described as authoritative but flagged as DHCP
**Status note:** Added VERIFY comment for Macca to convert to static reservation or move to mDNS hostname.
...
### 8. [VERIFY-NEEDED] SD-card-backup note may be obsolete
**Status note:** Added VERIFY comment for Noah to confirm the 16 GB card still exists/matters.
...
### 9. [VERIFY-NEEDED] ESP32 at `192.168.0.172` on the entropy network
**Status note:** Added VERIFY comment for Noah/Vlad; also tracked in the new Next-moves list.
...
### 10. [FIXED] "~2 unflashed AITRIP boards remain" — arithmetic is shaky
**Status note:** Corrected to ~3 (10 AITRIP − 7 AITRIP deployed). Kept VERIFY for Noah to physically count.
...
### 11. [VERIFY-NEEDED] Tailscale IPs presented without an "as of" date
**Status note:** Added VERIFY comment for Macca to stamp last-verified date.
...
### 12. [VERIFY-NEEDED] Credentials file `tailscale-authkey` flagged "empty/unused"
**Status note:** Replaced the row with a VERIFY comment; Noah to delete the physical file then remove the line.
...
### 13. [FIXED] `scripts/bootstrap-glinet-router.sh` deletion note is now ancient history
**Status note:** Pruned the memorialised-deletion sentence; only the canonical bootstrap script is mentioned now.
...
### 14. [STILL-OPEN] `docs/roadmap.md` is named in CLAUDE.md as the source of truth but is also stale
**Status note:** Out of scope for this ticket (it's a separate doc). Tracked as a Next-moves item for Sophie to drive a status pass.
...
### 15. [FIXED] "Pi runs Tailscale" persists despite branch activity
**Status note:** Already verified-true; no change to CLAUDE.md needed.
...
### 16. [FIXED] CLAUDE.md never mentions the `tools/ota_serve.py` helper
**Status note:** Added a bullet in the Current-state section pointing at `tools/ota_serve.py`.
...
### 17. [FIXED] CLAUDE.md never mentions `tests/`
**Status note:** Added a "How we test" section enumerating the test files + coverage audit doc. VERIFY left for the `docs/testing.md` pointer once `docs/test-pyramid` lands on main.
...
**17 items flagged.** After PRIYA-2 reconciliation pass:
- **[FIXED]: 10** — items 2, 3, 4, 5, 6, 10, 13, 15, 16, 17
- **[VERIFY-NEEDED]: 6** — items 1, 7, 8, 9, 11, 12 (HTML VERIFY comments inline in CLAUDE.md for the right human to confirm)
- **[STILL-OPEN]: 1** — item 14 (`docs/roadmap.md` status pass — out of scope, tracked in Next moves)

CLAUDE.md is now caught up to shipped work. The remaining gaps are all physical-world ("is this board still on the shelf", "did this IP renumber") and need a human at Noah's desk to close out.
