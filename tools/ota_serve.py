...
Usage (from the repo root):
    ...
    idf.py build
    python3 tools/ota_serve.py            # serves build/mockingbird.bin on :8000

Then, from any machine on the tailnet:

    curl -X POST http://<device-ts-ip>/ota \\
         -H "X-OTA-Token: <secret>" \\
         -d "{\\"url\\":\\"http://<your-ts-ip>:8000/mockingbird.bin\\"}"
...
def main() -> int:
    ...
    parser.add_argument(
        "binary",
        nargs="?",
        default="build/mockingbird.bin",
        help="path to firmware .bin (default: build/mockingbird.bin)",
    )
