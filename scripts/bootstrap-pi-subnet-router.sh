...
# mockingbird-pi role: Tailscale subnet router on Mockingbird WiFi,
...
if tailscale status >/dev/null 2>&1; then
    tailscale set --advertise-routes=192.168.8.0/24 --accept-routes --hostname=mockingbird-pi
    echo '    state:'
    tailscale status | head -3
else
    echo '    NOT YET LOGGED IN — run interactively:'
    echo '       sudo tailscale up --advertise-routes=192.168.8.0/24 --accept-routes --hostname=mockingbird-pi'
...
echo "(toggle on under 'Edit route settings' for mockingbird-pi)."
